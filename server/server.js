...
function start() {
	...
	__server = http.createServer(onRequest).on('close', onClose).on('clientError', onClientError).listen(__serverPort);

	__wss = new WebSocketServer({
		port : __websocketPort
	});

	__wss.broadcast = function(data) {
		O.d('Trying to broadcast data');
		O.t('          data: ' + JSON.stringify(data));
	...
	__wss.on('connection', function connection(conn) {
		conn.on('message', function incoming(message) {
			O.i('WebSocket connection recieved message: ' + message);
		});
...
function startTweetBroadcast() {
	if (__tpsRunning) {
		__tweetStream.stopTracking();
	}

	__tweetStream = new twitter.TwitterPublicStream({
	...
	__tweetStream.on('connected', function(response) {
		O.i('Connected to Twitter');
		...
		__wss.broadcast('tc');
		setTimeout(startTweetBroadcast, 5000);
		__tpsRunning = false;
	...
	__tweetStream.removeAllListeners('data').on('data', function(msg) {
		O.d('Parsed data!');
		// O.d(' On => ' + msg['created_at']);
		// var tweet = JSON.stringify(msg['text']);
		// O.d(' Tweet => ' + tweet);
		// O.d(' User => ' + msg['user']['name'] + '(@' +
		// msg['user']['screen_name'] + ')');
		O.t(' Full data => ' + JSON.stringify(msg));
		__wss.broadcast(JSON.stringify(msg));
	});

	__tweetStream.track('#election2016,#obama,#hillary,#clinton,#trump');
}
